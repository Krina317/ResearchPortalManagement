...
function collapseAuthorColumns(columns, mergeAuthors) {
    if (!mergeAuthors) return columns;
    const firstAuthorIndex = columns.findIndex(c => c.authorPosition);
    if (firstAuthorIndex === -1) return columns;
    const mergedColumn = { key: "authors", label: "Authors", field: "authors", sortable: false };
    const insertAt = columns.slice(0, firstAuthorIndex).filter(c => !c.authorPosition).length;
...
function getExportValue(record, column, mergeAuthors) {
    if (column.authorPosition) {
        ...
        const author = record.authors.find(a => a.authorPosition === column.authorPosition);
    ...
    if (column.key === "authors") {
        if (mergeAuthors) {
            ...
            if (Array.isArray(record.authors)) {
                return record.authors.map(a => a.displayName ?? a.name ?? "").filter(Boolean).join(", ");
            }
            return "";
        }
        if (Array.isArray(record.authors)) {
    ...
    if (Array.isArray(value)) {
        return value.map(item =>
            typeof item === "object" ? (item.displayName ?? item.name ?? JSON.stringify(item)) : item
        ).join(", ");
    }
...
export default function PublicationPage({ publicationType }) {
    ...
    const createDefaultFilters = useCallback(() => {
        ...
        publicationConfig.filters.forEach(filter => {
            ...
            } else if (filter.type === "dropdown" || filter.type === "text") {
    ...
    useEffect(() => {
        ...
        Promise.all([
            ...
            .catch(err => {
                ...
                setFilterOptions({});
                setColumns(publicationConfig.columns ?? []);
                setMetadataError("Could not load filter/column options from the server. Showing defaults.");
            });
    ...
    const loadData = useCallback(async () => {
        ...
        try {
            ...
            console.error(err);
            setError(`Could not load ${publicationConfig.title.toLowerCase()}. Is the backend running?`);
            setData(null);
        } finally {
            setLoading(false);
        }
    }, [publicationType, appliedFilters, page, pageSize, sortBy, sortDir, publicationConfig]);
    ...
    async function handleExport() {
        ...
        try {
            const allRecords = await fetchAllMatchingPublication(
                publicationType, appliedFilters, { sortBy, sortDir }
            );
            if (!allRecords || allRecords.length === 0) {
                alert(`No ${publicationConfig?.singularTitle?.toLowerCase() ?? "publication"} records match the current filters.`);
                return;
            }
            ...
            XLSX.utils.book_append_sheet(workbook, worksheet, publicationType || "Publication");
            const date = new Date().toISOString().slice(0, 10);
            XLSX.writeFile(workbook, `${publicationType || "publication"}-data-${date}.xlsx`);
        } catch (err) {
            console.error(err);
    ...
    if (!publicationConfig) {
        return (
            <div className="p-8">
                <h1 className="text-2xl font-semibold text-red-600">Invalid publication type</h1>
                <p className="mt-2 text-gray-500">No configuration exists for "{publicationType}".</p>
            </div>
        );
    }
    ...
    return (
        <div className="min-h-screen bg-gray-50">
            <div className="w-full max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-6">
                <PublicationHeader config={publicationConfig} />

                <PublicationToolbar
                    config={{ ...publicationConfig, columns }}
